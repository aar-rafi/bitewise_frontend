...
import { useRegister, useLogin, useVerifyLogin, useGoogleLogin } from "@/hooks/useAuth";
...
const Index = () => {
  ...
  const { googleLogin, googleLoginAsync, isLoading: isGoogleLoading, error: googleError } = useGoogleLogin({
    onSuccess: (data) => {
      // This callback is still useful for the mutation version
      console.log("Google OAuth URL received:", data);
    },
    onError: (error) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const response = await googleLoginAsync({ redirectUri });
      
      if (response?.authorization_url) {
        // Step 2: Redirect user to Google OAuth page
            ...
            <CardContent className="space-y-6 relative z-10">
              ...
              {registerError && (
                <Alert className="bg-red-50/50 border-red-200 backdrop-blur-sm">
                  ...
                  <AlertDescription className="text-red-800">
                    ...
                    {registerError.status === 422 && registerError.details && 
                     typeof registerError.details === 'object' && 
                     'detail' in registerError.details && (
                      <ul className="mt-2 list-disc list-inside text-sm">
                        {(registerError.details as ValidationError).detail.map((err, index) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <Button
                  ...
                  disabled={isLoading || isRegisterLoading || isLoginLoading || isVerifyLoginLoading}
                >
                  {isLoading ? (
